...
class UnlabeledNumpyDataset(torch.utils.data.Dataset):
    def __init__(self, root, transform=None):
        ...
        logger.info(f"synthetic data size: {self.size}")
    ...
    def hook_fn(self, module, input, output):
        ...
        if self.mmt is None:
            ...
            r_feature = torch.norm(
                ...
                - (1 - self.mmt_rate) * var
                - self.mmt_rate * var_mmt,
                2,
            ) + torch.norm(
                module.running_mean.data
                - (1 - self.mmt_rate) * mean
                - self.mmt_rate * mean_mmt,
                2,
            )
...
class Synthesizer:
    ...
    def synthesize(self, targets=None):
        ...
        best_inputs = None
        z = (
            torch.randn(size=(self.synthesis_batch_size, self.nz))
            .cuda()
            .requires_grad_()
        )
        if targets is None:
        ...
        tmp_data = (best_inputs.detach().clamp(0, 1).cpu().numpy() * 255).astype(
            "uint8"
        )
...
def _run_train(
    ...
    with run:
        ...
        weight = torch.load(
            directory_manager.get_teacher_model_path(
                args.teacher_dir, shadow_model_idx=shadow_model_idx
            )
        )
        ...
        if args.eval_only:
            ...
            logger.info("Loaded model, and evaluating")
...
def _train_model(
    ...
    optimizer = torch.optim.SGD(
        student_net.parameters(), lr=lr, momentum=momentum, weight_decay=weight_decay
    )
    lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, 200, eta_min=2e-4
    )
    ...
    for epoch in tqdm.tqdm(range(num_epochs)):
        ...
        if epoch >= 20:
            ...
            _train_student(
                syn_loader, student_net, teacher_net, criterion, optimizer
            )  # kd_steps
            lr_scheduler.step()
            student_net.eval()
            ...
            logger.info(
                f"student_net, epoch {epoch}, acc: {test_acc:.2f}, best: {best_acc:.2f}, test_loss: {test_loss:.2f}"
            )
...
class DirectoryManager(object):
    ...
    def get_teacher_model_path(
        self, teach_dir: str, shadow_model_idx: typing.Optional[int]
    ) -> pathlib.Path:
        teach_dir = pathlib.Path(teach_dir)
        return teach_dir / str(shadow_model_idx) / "model.pt"

    def get_training_img_dir(
        self, shadow_model_idx: typing.Optional[int]
    ) -> pathlib.Path:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--teacher_dir", type=str, default="", help="Teacher model directory"
    )
