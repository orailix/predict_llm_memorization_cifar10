...
def _train_model(
    ...
    if pretrain_epochs is not None:
        ...
        model, pretrain_metrics = _pretrain(
            pretrain_epochs, data_generator, training_seed, verbose
        )
    ...
    with warnings.catch_warnings():
        ...
        privacy_engine = dpsgd_utils.PrivacyEngineAugmented(
            opacus.GradSampleModule.GRAD_SAMPLERS
        )
    ...
    augmentation = dpsgd_utils.AugmentationMultiplicity(
        augmult_factor if augmult_factor > 0 else 1
    )
    model.GRAD_SAMPLERS[
    ...
    for epoch in (pbar := tqdm.trange(num_epochs, desc="Training", unit="epoch")):
        ...
        with opacus.utils.batch_memory_manager.BatchMemoryManager(
            ...
            for batch_xs, batch_ys in tqdm.tqdm(
                ...
                if augmult_factor > 0:
                    # Taken from tan code:
                    batch_xs = torch.repeat_interleave(
                        batch_xs, repeats=augmult_factor, dim=0
                    )
                    batch_ys = torch.repeat_interleave(
                        batch_ys, repeats=augmult_factor, dim=0
                    )
                    ...
                    batch_xs = torchvision.transforms.v2.Lambda(
                ...
                batch_unnormalized_accuracy = (
                    (batch_pred.detach().argmax(-1) == batch_ys).int().sum().item()
                )
...
def _pretrain(
    ...
    logger.info("Pre-training model on CIFAR-100")
    ...
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(
        optimizer, milestones=(100, 160), gamma=0.1
    )
    ...
    pretrained_test_metrics = {
        f"pretrain_{k}": v for k, v in pretrained_test_metrics.items()
    }
    logger.info(
        f"Pre-train test accuracy: {pretrained_test_metrics['pretrain_test_accuracy']:.4f}"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    # Defense-specific
    train_parser.add_argument(
        "--num-epochs", type=int, default=200, help="Number of training epochs"
    )
    train_parser.add_argument(
        "--noise-multiplier", type=float, default=0.2, help="Gaussian noise multiplier"
    )
    train_parser.add_argument(
        "--max-grad-norm", type=float, default=1.0, help="Gradient clipping norm"
    )
    train_parser.add_argument(
        "--learning-rate", type=float, default=4.0, help="Learning rate"
    )
    train_parser.add_argument(
        "--batch-size", type=int, default=2048, help="Virtual batch size"
    )
    train_parser.add_argument(
        "--augmult-factor",
        type=int,
    ...
    train_parser.add_argument(
        ...
        type=int,
        required=False,
        help="If set, pre-train for the given number of epochs",
    )

    # MIA
    attack_parser = subparsers.add_parser("attack")
    attack_parser.add_argument(
        "--global-variance", action="store_true", help="Use global variance for LiRA"
    )
