...
def _run_train(
    ...
    assert target_loss >= 0.0
    data_augmentation = args.data_augmentation
    num_epochs = 300

    logger.info(
        f"Training shadow model {shadow_model_idx}, target loss {target_loss}, data augmentation {data_augmentation}"
    )
    logger.info(
...
def _train_model(
    ...
    for epoch in (pbar := tqdm.trange(num_epochs, desc="Training", unit="epoch")):
        ...
        for batch_xs, batch_ys in tqdm.tqdm(
            ...
            batch_loss_ce = torch.mean(batch_loss_ce_full)

            batch_pred_ys = batch_pred.argmax(-1)

            # NB: original paper uses 1-based epochs, but their code uses 0-based epochs
            # NB: this branching follows the original code, but should be equivalent to the original paper's description
            if epoch % 2 == 0:
                batch_loss = (batch_loss_ce - target_loss).abs()
            else:
                if batch_loss_ce > target_loss:
                    ...
                    batch_pred_probs = torch.softmax(batch_pred, dim=-1)
                    batch_conf_targets = batch_pred_probs[
                        torch.arange(batch_ys.size(0)), batch_ys
                    ]
                    batch_conf_others = (1.0 - batch_conf_targets) / (num_classes - 1)
                    batch_soft_labels = batch_conf_others.unsqueeze(-1) * torch.ones(
                        (batch_ys.size(0), num_classes),
                        dtype=base.DTYPE,
                        device=base.DEVICE,
                    )
                    batch_soft_labels[
                    ...
                    batch_soft_labels = (
                        batch_soft_labels.detach()
                    )  # NB: original code has no detach!!

                    batch_loss_soft_full = loss(
                        input=batch_pred, target=batch_soft_labels
                    )

                    # Posterior flattening for INCORRECT predictions only (as in original paper)
                    # Also, follow original paper and perform gradient ASCEND for ALL samples (not just correct preds)
                    # FIXME: Doing ascent on all samples is a bit weird and not in the paper
                    #  3 options: i) original code, ii) soft-labels for all samples, iii) ascent only for correct preds
                    #  We follow the original code here
                    batch_pred_correct = (batch_pred_ys == batch_ys).float()
                    batch_loss = torch.mean(
                        (1.0 - batch_pred_correct) * batch_loss_soft_full
                        - batch_loss_ce_full
                    )
                    # batch_loss = torch.mean(batch_loss_soft_full)
                    # batch_loss = torch.mean(torch.where(batch_pred_correct, -batch_loss_ce_full, batch_loss_soft_full))

            batch_loss.backward()
            optimizer.step()

            epoch_loss += batch_loss.item() * batch_xs.size(0)
            epoch_loss_ce += batch_loss_ce.item() * batch_xs.size(0)
            epoch_accuracy += (batch_pred_ys == batch_ys).int().sum().item()
            num_samples += batch_xs.size(0)
        epoch_loss /= num_samples
        epoch_loss_ce /= num_samples
...
def parse_args() -> argparse.Namespace:
    ...
    # Defense-specific
    train_parser.add_argument(
        "--target-loss", type=float, default=1.0, help="Target loss (alpha)"
    )
    train_parser.add_argument(
        "--data-augmentation",
        action="store_true",
