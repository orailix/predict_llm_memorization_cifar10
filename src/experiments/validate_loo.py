...
def main():
    ...
    assert num_poison >= 0
    assert args.fixed_halves in ("true", "false")
    fixed_halves = args.fixed_halves == "true"
    ...
    if args.action == "attack":
        ...
        _run_attack(args, data_generator, directory_manager, fixed_halves=fixed_halves)
...
def _run_attack(
    ...
    # Will perform attacks both on just canary subset and full dataset
    score_maps = {
        "canary subset": (shadow_scores, shadow_membership_mask[canary_indices], ""),
    }
    if not fixed_halves:
        score_maps["full dataset"] = (
            shadow_scores_full,
            shadow_membership_mask,
            "_full",
        )
    ...
    for score_map_name, (
        ...
        logger.info(f"## on {score_map_name}")
        for score_name, scores in score_map.items():
            ...
            _eval_attack(
                ...
                attack_membership=membership_mask.view(-1),
                output_dir=output_dir,
                suffix=f"global_{score_name}{score_map_suffix}",
            )
        ...
        for score_map_name, (
            ...
            if is_augmented:
                shadow_attack_data = {
                    score_name: attack_util.lira_attack_loo(
                        ...
                        shadow_membership_mask=membership_mask,
                    )
                    for score_name, scores in score_map.items()
                }
            else:
            ...
            for score_name, (scores, membership) in shadow_attack_data.items():
                ...
                _eval_attack(
                    ...
                    output_dir=output_dir,
                    suffix=f"{attack_suffix}_{score_name}{score_map_suffix}",
                )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        required=True,
        choices=["true", "false"],
        help="If true, only split canaries into IN vs. OUT, else the full training dataset",
    )
