...
def setup_seeds(
    ...
    if deterministic_algorithms:
        ...
        if benchmark_algorithms:
            raise ValueError(
                "Benchmarking should not be enabled under deterministic algorithms"
            )
...
def lira_attack(
    ...
    shadow_in_indices = torch.stack(
        [torch.argwhere(mask).squeeze(-1) for mask in shadow_membership_mask], dim=0
    )
    shadow_out_indices = torch.stack(
        [torch.argwhere(mask).squeeze(-1) for mask in ~shadow_membership_mask], dim=0
    )
    scores_in = torch.gather(
        shadow_scores,
        dim=1,
        index=shadow_in_indices.unsqueeze(-1).tile((1, 1, shadow_scores.size(-1))),
    )
    scores_out = torch.gather(
        shadow_scores,
        dim=1,
        index=shadow_out_indices.unsqueeze(-1).tile((1, 1, shadow_scores.size(-1))),
    )
    ...
    log_prs_in = np.mean(
        scipy.stats.norm.logpdf(target_scores, means_in, stds_in), axis=-1
    )
    log_prs_out = np.mean(
        scipy.stats.norm.logpdf(target_scores, means_out, stds_out), axis=-1
    )
...
def lira_attack_loo(
    ...
    full_attack_scores = torch.empty(
        num_samples * num_shadow_models, dtype=LIRA_DTYPE_TORCH
    )
    ...
    for target_model_idx in range(num_shadow_models):
        ...
        current_shadow_scores = torch.cat(
            (
                shadow_scores[:, :target_model_idx],
                shadow_scores[:, target_model_idx + 1 :],
            ),
            dim=1,
        )
        assert current_shadow_scores.size() == (
            num_samples,
            num_shadow_models - 1,
            num_augmentations,
        )
        current_shadow_membership = torch.cat(
            (
                ...
                shadow_membership_mask[:, target_model_idx + 1 :],
            ),
            dim=1,
        )
        ...
        if global_variance:
            current_full_mask = torch.tile(
                current_shadow_membership.unsqueeze(0), (num_augmentations, 1, 1)
            )
            scores_in = (
                current_shadow_scores.permute(2, 0, 1)[current_full_mask]
                .view(num_augmentations, -1)
                .permute(1, 0)
            )
            scores_out = (
                current_shadow_scores.permute(2, 0, 1)[~current_full_mask]
        ...
        for sample_idx in range(num_samples):
            scores_in = current_shadow_scores[
                sample_idx, current_shadow_membership[sample_idx]
            ]
            scores_out = current_shadow_scores[
                sample_idx, ~current_shadow_membership[sample_idx]
            ]
        ...
        full_attack_scores[
            target_model_idx * num_samples : (target_model_idx + 1) * num_samples
        ] = torch.from_numpy(result_scores).to(dtype=LIRA_DTYPE_TORCH)
...
def logit_score_from_probs(
    probs_predictions: torch.Tensor, labels: torch.Tensor, eps: float = 1e-30
) -> torch.Tensor:
    assert (
        probs_predictions.dim() >= 2
        and labels.dim() == 1
        and probs_predictions.size(0) == len(labels)
    )
    ...
    return torch.log(probs_target + eps) - torch.log(
        torch.sum(probs_predictions, dim=-1) + eps
    )
