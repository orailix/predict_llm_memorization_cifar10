...
class Dataset(torch.utils.data.Dataset):
    ...
    def _getitem_with_idx(
        self, index: int
    ) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        # Here, we MUST have labels
        target_idx = self._indices[index]
        return (
            torch.Tensor([target_idx]).to(dtype=int),
            self._features[target_idx],
            self._labels[target_idx],
        )
    ...
    def build_datapipe(
        ...
        # In case of global IDX, we need to modify the mapping
        if add_global_index:
            datapipe = datapipe.map(self._getitem_with_idx)
        else:
...
class DatasetGenerator(object):
    ...
    def __init__(
        ...
        seed: int,
        download: bool = False,
        fixed_halves: typing.Optional[
            bool
        ] = None,  # this is just for validation of the setting
    ) -> None:
        ...
        self._clean_train_xs, self._clean_train_ys = self._load_cifar10(
            data_dir, train=True, download=download
        )
        self._test_xs, self._test_ys = self._load_cifar10(
        ...
        if self._canary_type == CanaryType.OOD:
            ...
            if not ood_samples_path.exists():
                raise FileNotFoundError(
                    f"OOD canary image file {ood_samples_path} does not exist"
                )
            ood_samples_signature = hashlib.md5(
                ood_samples_path.read_bytes()
            ).hexdigest()
            ...
            if self._ood_xs.size(0) < num_canaries:
                raise ValueError(
                    f"Requested {num_canaries} OOD canaries, but only {self._ood_xs.size(0)} available"
                )
        elif self._canary_type == CanaryType.SSL_WORST_CASE:
            ssl_indices_path = data_dir / "ssl_indices.pt"
            if not ssl_indices_path.exists():
                raise FileNotFoundError(
                    f"SSL indices file {ssl_indices_path} does not exist"
                )
            ssl_indices_signature = hashlib.md5(
                ssl_indices_path.read_bytes()
            ).hexdigest()
        ...
        shadow_in_indices_t = np.argsort(
            rng_splits_shadow.uniform(size=(self._num_shadow, num_raw_train_samples)),
            axis=0,
        ...
        for shadow_idx in range(self._num_shadow):
            raw_shadow_in_indices.append(
                torch.from_numpy(
                    np.argwhere(
                        np.any(shadow_in_indices_t == shadow_idx, axis=1)
                    ).flatten()
                )
            )
        rng_splits_half, rng_splits_shadow = rng_splits_shadow.spawn(
            2
        )  # used later for fixed splits for validation
        ...
        if fixed_halves is None:
            ...
            for shadow_idx in range(self._num_shadow):
                ...
                self._shadow_in_indices.append(
                    torch.argwhere(current_in_mask).flatten()
                )
        else:
            ...
            if not fixed_halves:
                ...
                canary_mask = torch.zeros(num_raw_train_samples, dtype=torch.bool)
                canary_mask[self._canary_order[: self._num_canaries]] = True
                fixed_membership_full = torch.from_numpy(
                    rng_splits_half.random(num_raw_train_samples) < 0.5
                )
                for shadow_idx in range(self._num_shadow):
                    current_in_mask = torch.zeros(
                        num_raw_train_samples, dtype=torch.bool
                    )
        ...
        label_changes = torch.from_numpy(
            rng_noise.integers(num_classes - 1, size=num_raw_train_samples)
        )
        self._noisy_labels = torch.where(
            label_changes < self._clean_train_ys, label_changes, label_changes + 1
        )
        del rng_noise
        ...
        self._poison_random_images_seed = rng_poison_images.integers(
        ...
        self._poison_duplicate_label_changes = torch.from_numpy(
            rng_poison_duplicates_labels.integers(
                num_classes - 1, size=num_raw_train_samples
            )
        )
    ...
    def build_attack_data_with_poison(
        ...
        shadow_membership_mask = torch.zeros(
            (len(attack_ys), self._num_shadow), dtype=torch.bool
        )
        for shadow_model_idx in range(self._num_shadow):
            shadow_membership_mask[
                self._shadow_in_indices[shadow_model_idx], shadow_model_idx
            ] = True
        ...
        if self._canary_type != CanaryType.DUPLICATES_MISLABEL_HALF:
            ...
            canary_indices_mislabel = self.get_canary_indices()[
                -self._num_canaries // 2 :
            ]
            ...
            shadow_membership_mask_new[
                canary_indices_mislabel
            ] = shadow_membership_mask[canary_indices_clean]
            shadow_membership_mask_new[canary_indices_clean] = shadow_membership_mask[
                canary_indices_mislabel
            ]
    ...
    def build_attack_data(
        ...
        if self._canary_type == CanaryType.DUPLICATES_MISLABEL_HALF:
            assert (
                len(canary_indices) % 2 == 0
            ), "Need even number of canaries to use half"
            canary_indices = canary_indices[-self._num_canaries // 2 :]

        # As usual, this mask does not include poisons
        shadow_membership_mask = torch.zeros(
            (self.num_raw_training_samples, self._num_shadow), dtype=torch.bool
        )
        for shadow_model_idx in range(self._num_shadow):
            shadow_membership_mask[:, shadow_model_idx] = self.build_in_mask(
                shadow_model_idx
            )
    ...
    def _build_train_data(
        ...
        if include_poison and self._num_poison > 0:
            if self._poison_type == PoisonType.RANDOM_IMAGES:
                ...
                poison_indices = torch.arange(
                    len(train_xs), len(train_xs) + len(poison_xs)
                )
                ...
                assert (
                    self._num_poison == self._num_canaries
                ), "Currently only support duplicating every canary"
                ...
                poison_ys = torch.where(
                    label_changes < canary_ys, label_changes, label_changes + 1
                )
                poison_indices = torch.arange(
                    len(train_xs), len(train_xs) + len(poison_xs)
                )
                ...
                poison_ys = torch.where(
                    label_changes < duplicate_ys, label_changes, label_changes + 1
                )
                poison_indices = torch.arange(
                    len(train_xs), len(train_xs) + len(poison_xs)
                )
    ...
    def _build_train_canary_data(self) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self._canary_type == CanaryType.CLEAN:
            ...
            return self._clean_train_xs, self._clean_train_ys
        elif self._canary_type in (
            CanaryType.DUPLICATES_MISLABEL_HALF,
            CanaryType.DUPLICATES_MISLABEL_FULL,
        ):
            canary_features = self._clean_train_xs.clone()
            canary_targets = self._clean_train_ys.clone()
            canary_indices = self.get_canary_indices()
            ...
            canary_features[canary_indices_duplicate] = canary_features[
                canary_indices_original
            ]
            # Always mislabel half of canaries (need to use original noisy labels)
            canary_targets[canary_indices_duplicate] = self._noisy_labels[
                canary_indices_original
            ]
            if self._canary_type == CanaryType.DUPLICATES_MISLABEL_FULL:
                # Also use same wrong labels for other half
                canary_targets[canary_indices_original] = canary_targets[
                    canary_indices_duplicate
                ]
            return canary_features, canary_targets
        else:
    ...
    def _build_poison_random_images(self) -> typing.Tuple[torch.Tensor, torch.Tensor]:
        ...
        poison_targets = torch.tile(
            torch.arange(num_classes, dtype=torch.long), (num_random_images,)
        )
        poison_features = torch.zeros((num_poison_total, 3, 32, 32), dtype=torch.uint8)
        for idx in range(num_random_images):
            poison_features[
                idx * num_classes : (idx + 1) * num_classes
            ] = torch.from_numpy(
