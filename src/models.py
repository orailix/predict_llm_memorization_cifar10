...
class WideBlock(torch.nn.Module):
    def __init__(
        ...
        norm_builder: typing.Callable[
            [int, typing.Optional[torch.device], typing.Optional[torch.dtype]],
            torch.nn.Module,
        ],
...
class WideResNet(torch.nn.Module):
    def __init__(
        ...
        if use_group_norm:
            ...
            def norm_builder(
                ...
                return torch.nn.GroupNorm(
                    min(16, num_features), num_features, device=device, dtype=dtype
                )
        ...
        self.dense = torch.nn.Linear(
            self._dense_features_in, num_classes, bias=True, device=device, dtype=dtype
        )
    ...
    def forward(
        ...
        # Pool and flatten
        x = self.flatten(self.pool(x))  # Shape (bs, 256)

        # Return hidden repr without dense network
        if return_hidden:
            return x
